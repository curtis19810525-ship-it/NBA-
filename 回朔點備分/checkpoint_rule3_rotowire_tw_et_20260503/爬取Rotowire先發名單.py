...
def _prompt_date_range():
    while True:
        start = input("Enter Start Date (YYYYMMDD): ").strip()
        end = input("Enter End Date (YYYYMMDD): ").strip()
        try:
...
def _parse_args():
    ...
    raise ValueError("參數格式錯誤。請使用：YYYYMMDD YYYYMMDD 或 YYYYMMDD~YYYYMMDD")
...
def _extract_games_from_soup(soup, target_date):
    rows = []
...
def _fetch_date_rows(target_date):
    url, mode = _build_url(target_date)
    resp = requests.get(url, headers=HTTP_HEADERS, timeout=25)
    resp.raise_for_status()
    soup = BeautifulSoup(resp.text, "html.parser")
    if not _page_matches_target(soup, target_date, mode):
        return None, f"來源頁面沒有 {target_date.strftime('%Y-%m-%d')} 可用內容"
    rows = _extract_games_from_soup(soup, target_date)
    return rows, None
...
def main():
    print("=" * 60)
    print("爬取 Rotowire MLB 先發名單（日期範圍）")
    print("=" * 60)
