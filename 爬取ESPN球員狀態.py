"""
爬取 ESPN MLB 球員資料。
- 打擊：team stats（All Splits + Expanded）→「球員狀態」分頁（每次覆蓋）
- 投手：team stats type=pitching（Player Pitching + Batting Against）→「各隊投手」分頁（每次覆蓋）
- 非 --injuries-only 時：先寫球員狀態，再寫各隊投手。
"""
...
# ESPN 欄名（與 _normalize_header 後一致）；Excel 第 1 列與寫入順序相同
PITCH_MAIN_KEYS = [
    "GP", "GS", "QS", "W", "L", "SV", "HLD", "IP", "H", "ER", "HR", "BB", "K",
    "K/9", "P/S", "WAR", "WHIP", "ERA",
]
PITCH_BATTING_AGAINST_KEYS = [
    "GP", "AB", "R", "H", "2B", "3B", "HR", "BB", "K", "SB", "CS", "OBA",
]

PITCHER_TARGET_HEADERS = (
    ["球隊", "投手姓名", "SP/RP"] + PITCH_MAIN_KEYS + PITCH_BATTING_AGAINST_KEYS
)
...
def _find_pitching_page_tables(soup):
    """投球頁：Player Pitching（NAME+統計）、Batting Against（NAME+統計）各一組。"""
    pitching_rows = []
    ba_rows = []

    pitching_need = set(PITCH_MAIN_KEYS)
    ba_need = set(PITCH_BATTING_AGAINST_KEYS)
    ...
    for i in range(len(tables) - 1):
        h1 = headers_list[i]
        h2 = headers_list[i + 1]
        if h1 == ["NAME"] and pitching_need.issubset(set(h2)) and not pitching_rows:
            name_rows = _extract_rows_with_headers(tables[i])
            stat_rows = _extract_rows_with_headers(tables[i + 1])
            pitching_rows = _pair_name_and_stats(name_rows, stat_rows)
        if h1 == ["NAME"] and ba_need.issubset(set(h2)) and not ba_rows:
            name_rows = _extract_rows_with_headers(tables[i])
            stat_rows = _extract_rows_with_headers(tables[i + 1])
            ba_rows = _pair_name_and_stats(name_rows, stat_rows)

    return pitching_rows, ba_rows


def fetch_team_pitching_rows(team_cn, abbr, slug):
    url = (
        f"https://www.espn.com/mlb/team/stats/_/type/pitching/name/{abbr}/{slug}"
    )
    try:
        html = _fetch_espn_html(url, team_cn)
    except Exception as e:
        print(f"  投手頁抓取失敗 {team_cn}: {e}")
        return []

    soup = BeautifulSoup(html, "html.parser")
    pitching_rows, ba_rows = _find_pitching_page_tables(soup)
    if not pitching_rows:
        print(f"  未找到 Player Pitching 表：{team_cn}")
        return []

    if len(ba_rows) != len(pitching_rows):
        print(
            f"  注意：Batting Against 列數（{len(ba_rows)}）與投手表（{len(pitching_rows)}）"
            f"不一致，已依列序對齊（缺則留空）：{team_cn}"
        )

    out = []
    for i, prow in enumerate(pitching_rows):
        brow = ba_rows[i] if i < len(ba_rows) else {}
        player = (prow.get("NAME", "") or "").strip()
        pos = (prow.get("POS", "") or "").strip()
        out.append(
            [team_cn, player, pos]
            + [prow.get(k, "") for k in PITCH_MAIN_KEYS]
            + [brow.get(k, "") for k in PITCH_BATTING_AGAINST_KEYS]
        )
    return out
...
def write_pitchers_sheet(all_rows):
    if not os.path.exists(NBA_STATS_FILE):
        raise FileNotFoundError(f"Excel 檔案不存在：{NBA_STATS_FILE}")

    wb = load_workbook(NBA_STATS_FILE)
    if SHEET_PITCHERS not in wb.sheetnames:
        wb.create_sheet(SHEET_PITCHERS)
    ws = wb[SHEET_PITCHERS]

    for idx, title in enumerate(PITCHER_TARGET_HEADERS, start=1):
        ws.cell(row=1, column=idx, value=title)
...
def main():
    ...
    if injuries_only:
        ...
        print()
        print("=" * 60)
        print("各隊投手（ESPN type=pitching：主投手 + Batting Against）")
        print("=" * 60)
        pitch_rows_all = []
        for i, (team_cn, abbr, slug) in enumerate(MLB_TEAMS, start=1):
            print(f"[{i}/{total}] {team_cn} 投手...")
            prow = fetch_team_pitching_rows(team_cn, abbr, slug)
            pitch_rows_all.extend(prow)
            time.sleep(0.25)
        print(f"\n總計投手列：{len(pitch_rows_all)} 筆")
        write_pitchers_sheet(pitch_rows_all)
        print(f"完成：已覆蓋寫入「{SHEET_PITCHERS}」分頁。")
